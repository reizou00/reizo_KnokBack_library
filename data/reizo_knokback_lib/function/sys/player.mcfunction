...
#> 計算のために1000倍したのでそれを0.00001倍して元の値に戻す。
    # X座標
    execute \
    store result storage reizo_knokback_lib:_ x int 0.1 run \
    scoreboard players get $KnokBackX reizo_KnokBack_lib.Temp
    # Y座標
    execute \
    store result storage reizo_knokback_lib:_ y int 0.1 run \
    scoreboard players get $KnokBackY reizo_KnokBack_lib.Temp
    # Z座標
    execute \
    store result storage reizo_knokback_lib:_ z int 0.1 run \
